...
describe('FetchCategoryById', () => {
  ...
  it('should throw an error when fetchCategoryById execute method fails', async () => {
    ...
    mockCategoryService.fetchCategoryById.mockRejectedValue("Service method error");

    // Verify that the use case throws an error when service method fails
    await expect(fetchCategoryById.execute(id)).rejects.toThrow('Service method error');
  });
});
