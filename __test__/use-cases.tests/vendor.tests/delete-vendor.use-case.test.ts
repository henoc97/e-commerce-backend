...
/**
 * Test suite for the DeleteVendor use case.
 * This test covers the initialization and behavior of the DeleteVendor class.
 * It mocks the VendorService service and verifies that the use case handles the business logic as expected.
 */
describe('DeleteVendor', () => {
  let deleteVendor: DeleteVendor;
  ...
  beforeEach(async () => {
    ...
    deleteVendor = module.get<DeleteVendor>(DeleteVendor);
  ...
  it('should create and return an address DTO', async () => {
    ...
    mockVendorService.deleteVendor.mockResolvedValue(mockVendorDTO);
    (toVendorDTO as jest.Mock).mockReturnValue(mockVendorDTO);

    // Execute the use case with provided parameters
    const result = await deleteVendor.execute(vendorId);
