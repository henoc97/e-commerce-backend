...
/**
 * Test suite for the GetLatestVendor use case.
 * This test covers the initialization and behavior of the GetLatestVendor class.
 * It mocks the VendorService service and verifies that the use case handles the business logic as expected.
 */
describe('GetLatestVendor', () => {
  let getLatestVendor: GetLatestVendor;
  ...
  beforeEach(async () => {
    ...
    getLatestVendor = module.get<GetLatestVendor>(GetLatestVendor);
  ...
  it('should create and return an address DTO', async () => {
    ...
    mockVendorService.getLatestVendor.mockResolvedValue(mockVendorDTO);
    (toVendorDTO as jest.Mock).mockReturnValue(mockVendorDTO);

    // Execute the use case with provided parameters
    const result = await getLatestVendor.execute();

    // Verify that the service was called with the expected arguments
    expect(mockVendorService.getLatestVendor).toHaveBeenCalledWith();
   ...
   */
  it('should throw an error when getLatestVendor execute method fails', async () => {
    // Simulate a failure when calling the service method
    mockVendorService.getLatestVendor.mockRejectedValue("Service method error");

    // Verify that the use case throws an error when service method fails
    await expect(getLatestVendor.execute()).rejects.toThrow('Service method error');
  });
});
