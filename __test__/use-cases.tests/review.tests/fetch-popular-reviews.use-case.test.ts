...
/**
 * Test suite for the FetchPopularReviews use case.
 * This test covers the initialization and behavior of the FetchPopularReviews class.
...
describe('FetchPopularReviews', () => {
  ...
  beforeEach(async () => {
    ...
    fetchPopularReviews = module.get<FetchPopularReviews>(FetchPopularReviews);
  ...
  it('should create and return an address DTO', async () => {
    ...
    mockReviewService.fetchPopularReviews.mockResolvedValue(mockReviewDTO);
    (toReviewDTO as jest.Mock).mockReturnValue(mockReviewDTO);

    // Execute the use case with provided parameters
    const result = await fetchPopularReviews.execute(limit);

    // Verify that the service was called with the expected arguments
    expect(mockReviewService.fetchPopularReviews).toHaveBeenCalledWith(limit);
   ...
   */
  it('should throw an error when fetchPopularReviews execute method fails', async () => {
    // Simulate a failure when calling the service method
    mockReviewService.fetchPopularReviews.mockRejectedValue("Service method error");

    // Verify that the use case throws an error when service method fails
    await expect(fetchPopularReviews.execute(limit)).rejects.toThrow('Service method error');
  });
});
