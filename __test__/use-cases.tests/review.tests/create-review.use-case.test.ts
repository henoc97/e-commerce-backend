...
/**
 * Test suite for the CreateReview use case.
 * This test covers the initialization and behavior of the CreateReview class.
...
describe('CreateReview', () => {
  ...
  beforeEach(async () => {
    ...
    createReview = module.get<CreateReview>(CreateReview);
  ...
  it('should create and return an address DTO', async () => {
    ...
    mockReviewService.createReview.mockResolvedValue(mockReviewDTO);
    (toReviewDTO as jest.Mock).mockReturnValue(mockReviewDTO);

    // Execute the use case with provided parameters
    const result = await createReview.execute(reviewDTO);

    // Verify that the service was called with the expected arguments
    expect(mockReviewService.createReview).toHaveBeenCalledWith(reviewDTO);
