...
/**
 * Test suite for the FetchReviewsByRating use case.
 * This test covers the initialization and behavior of the FetchReviewsByRating class.
 * It mocks the ReviewService service and verifies that the use case handles the business logic as expected.
 */
describe('FetchReviewsByRating', () => {
  let fetchReviewsByRating: FetchReviewsByRating;
  let reviewService: ReviewService;

  // Mock implementation of the ReviewService service with jest functions
  const mockReviewService = {
    fetchReviewsByRating: jest.fn(),
  };
  ...
  it('should create and return an address DTO', async () => {
    ...
    mockReviewService.fetchReviewsByRating.mockResolvedValue(mockReviewDTO);
    (toReviewDTO as jest.Mock).mockReturnValue(mockReviewDTO);

    // Execute the use case with provided parameters
    const result = await fetchReviewsByRating.execute(rating);

    // Verify that the service was called with the expected arguments
    expect(mockReviewService.fetchReviewsByRating).toHaveBeenCalledWith(rating);
