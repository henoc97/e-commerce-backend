...
describe('ListActiveSubsites', () => {
  ...
  it('should throw an error when listActiveSubsites execute method fails', async () => {
    ...
    mockSubsiteService.listActiveSubsites.mockRejectedValue("Service method error");

    // Verify that the use case throws an error when service method fails
    await expect(listActiveSubsites.execute()).rejects.toThrow('Service method error');
  });
});
