...
/**
 * Test suite for the ValidateSubsite use case.
 * This test covers the initialization and behavior of the ValidateSubsite class.
 * It mocks the SubsiteService service and verifies that the use case handles the business logic as expected.
 */
describe('ValidateSubsite', () => {
  let validateSubsite: ValidateSubsite;
  ...
  beforeEach(async () => {
    ...
    validateSubsite = module.get<ValidateSubsite>(ValidateSubsite);
  ...
  it('should create and return an address DTO', async () => {
    ...
    mockSubsiteService.validateSubsite.mockResolvedValue(mockSubsiteDTO);
    (toSubsiteDTO as jest.Mock).mockReturnValue(mockSubsiteDTO);

    // Execute the use case with provided parameters
    const result = await validateSubsite.execute(subsiteDTO);

    // Verify that the service was called with the expected arguments
    expect(mockSubsiteService.validateSubsite).toHaveBeenCalledWith(subsiteDTO);
