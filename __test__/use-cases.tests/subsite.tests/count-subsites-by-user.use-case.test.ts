...
/**
 * Test suite for the CountSubsitesByUser use case.
 * This test covers the initialization and behavior of the CountSubsitesByUser class.
 * It mocks the SubsiteService service and verifies that the use case handles the business logic as expected.
 */
describe('CountSubsitesByUser', () => {
  let countSubsitesByUser: CountSubsitesByUser;
  ...
  beforeEach(async () => {
    ...
    countSubsitesByUser = module.get<CountSubsitesByUser>(CountSubsitesByUser);
  ...
  it('should create and return an address DTO', async () => {
    ...
    mockSubsiteService.countSubsitesByUser.mockResolvedValue(mockSubsiteDTO);
    (toSubsiteDTO as jest.Mock).mockReturnValue(mockSubsiteDTO);

    // Execute the use case with provided parameters
    const result = await countSubsitesByUser.execute(userId);

    // Verify that the service was called with the expected arguments
    expect(mockSubsiteService.countSubsitesByUser).toHaveBeenCalledWith(userId);
   ...
   */
  it('should throw an error when countSubsitesByUser execute method fails', async () => {
    // Simulate a failure when calling the service method
    mockSubsiteService.countSubsitesByUser.mockRejectedValue("Service method error");

    // Verify that the use case throws an error when service method fails
    await expect(countSubsitesByUser.execute(userId)).rejects.toThrow('Service method error');
  });
});
