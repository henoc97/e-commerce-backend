...
/**
 * Test suite for the RemoveSubsite use case.
 * This test covers the initialization and behavior of the RemoveSubsite class.
 * It mocks the SubsiteService service and verifies that the use case handles the business logic as expected.
 */
describe('RemoveSubsite', () => {
  let removeSubsite: RemoveSubsite;
  ...
  beforeEach(async () => {
    ...
    removeSubsite = module.get<RemoveSubsite>(RemoveSubsite);
  ...
  it('should create and return an address DTO', async () => {
    ...
    mockSubsiteService.removeSubsite.mockResolvedValue(mockSubsiteDTO);
    (toSubsiteDTO as jest.Mock).mockReturnValue(mockSubsiteDTO);

    // Execute the use case with provided parameters
    const result = await removeSubsite.execute(id);

    // Verify that the service was called with the expected arguments
    expect(mockSubsiteService.removeSubsite).toHaveBeenCalledWith(id);
