...
/**
 * Test suite for the AddProductToShop use case.
 * This test covers the initialization and behavior of the AddProductToShop class.
 * It mocks the ShopService service and verifies that the use case handles the business logic as expected.
 */
describe('AddProductToShop', () => {
  let addProductToShop: AddProductToShop;
  ...
  beforeEach(async () => {
    ...
    addProductToShop = module.get<AddProductToShop>(AddProductToShop);
  ...
  it('should create and return an address DTO', async () => {
    ...
    mockShopService.addProductToShop.mockResolvedValue(mockShopDTO);
    (toShopDTO as jest.Mock).mockReturnValue(mockShopDTO);

    // Execute the use case with provided parameters
    const result = await addProductToShop.execute(shopId, productDTO);

    // Verify that the service was called with the expected arguments
    expect(mockShopService.addProductToShop).toHaveBeenCalledWith(
      shopId,
      productDTO,
    );
   ...
   */
  it('should throw an error when addProductToShop execute method fails', async () => {
    // Simulate a failure when calling the service method
    mockShopService.addProductToShop.mockRejectedValue('Service method error');

    // Verify that the use case throws an error when service method fails
    await expect(addProductToShop.execute(shopId, productDTO)).rejects.toThrow(
