...
/**
 * Test suite for the FetchShopList use case.
 * This test covers the initialization and behavior of the FetchShopList class.
...
describe('FetchShopList', () => {
  ...
  beforeEach(async () => {
    ...
    fetchShopList = module.get<FetchShopList>(FetchShopList);
  ...
  it('should create and return an address DTO', async () => {
    ...
    mockShopService.fetchShopList.mockResolvedValue(mockShopDTO);
    (toShopDTO as jest.Mock).mockReturnValue(mockShopDTO);

    // Execute the use case with provided parameters
    const result = await fetchShopList.execute();

    // Verify that the service was called with the expected arguments
    expect(mockShopService.fetchShopList).toHaveBeenCalledWith();
   ...
   */
  it('should throw an error when fetchShopList execute method fails', async () => {
    // Simulate a failure when calling the service method
    mockShopService.fetchShopList.mockRejectedValue("Service method error");

    // Verify that the use case throws an error when service method fails
    await expect(fetchShopList.execute()).rejects.toThrow('Service method error');
  });
});
