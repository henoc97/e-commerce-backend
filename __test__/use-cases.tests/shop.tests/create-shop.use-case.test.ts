...
/**
 * Test suite for the CreateShop use case.
 * This test covers the initialization and behavior of the CreateShop class.
...
describe('CreateShop', () => {
  ...
  beforeEach(async () => {
    ...
    createShop = module.get<CreateShop>(CreateShop);
  ...
  it('should create and return an address DTO', async () => {
    ...
    mockShopService.createShop.mockResolvedValue(mockShopDTO);
    (toShopDTO as jest.Mock).mockReturnValue(mockShopDTO);

    // Execute the use case with provided parameters
    const result = await createShop.execute(shopDTO);

    // Verify that the service was called with the expected arguments
    expect(mockShopService.createShop).toHaveBeenCalledWith(shopDTO);
   ...
   */
  it('should throw an error when createShop execute method fails', async () => {
    // Simulate a failure when calling the service method
    mockShopService.createShop.mockRejectedValue("Service method error");

    // Verify that the use case throws an error when service method fails
    await expect(createShop.execute(shopDTO)).rejects.toThrow('Service method error');
  });
});
