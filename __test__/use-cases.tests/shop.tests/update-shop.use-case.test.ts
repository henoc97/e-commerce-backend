...
import { UpdateShop } from '../../../src/application/use-cases/shop.use-cases/update-shop.use-case';
import { ShopDTO } from '../../../src/presentation/dtos/shop.dto';
import { toShopDTO } from '../../../src/application/helper/to-dto/to.address.dto';

/**
 * Test suite for the UpdateShop use case.
 * This test covers the initialization and behavior of the UpdateShop class.
 * It mocks the ShopService service and verifies that the use case handles the business logic as expected.
 */
describe('UpdateShop', () => {
  let updateShop: UpdateShop;
  ...
  beforeEach(async () => {
    ...
    updateShop = module.get<UpdateShop>(UpdateShop);
  ...
  it('should create and return an address DTO', async () => {
    ...
    mockShopService.updateShop.mockResolvedValue(mockShopDTO);
    (toShopDTO as jest.Mock).mockReturnValue(mockShopDTO);

    // Execute the use case with provided parameters
    const result = await updateShop.execute(shopId, updates);

    // Verify that the service was called with the expected arguments
    expect(mockShopService.updateShop).toHaveBeenCalledWith(shopId, updates);
