...
/**
 * Test suite for the ListShopsByVendor use case.
 * This test covers the initialization and behavior of the ListShopsByVendor class.
 * It mocks the ShopService service and verifies that the use case handles the business logic as expected.
 */
describe('ListShopsByVendor', () => {
  let listShopsByVendor: ListShopsByVendor;
  ...
  beforeEach(async () => {
    ...
    listShopsByVendor = module.get<ListShopsByVendor>(ListShopsByVendor);
  ...
  it('should create and return an address DTO', async () => {
    ...
    mockShopService.listShopsByVendor.mockResolvedValue(mockShopDTO);
    (toShopDTO as jest.Mock).mockReturnValue(mockShopDTO);

    // Execute the use case with provided parameters
    const result = await listShopsByVendor.execute(vendorId);

    // Verify that the service was called with the expected arguments
    expect(mockShopService.listShopsByVendor).toHaveBeenCalledWith(vendorId);
   ...
   */
  it('should throw an error when listShopsByVendor execute method fails', async () => {
    // Simulate a failure when calling the service method
    mockShopService.listShopsByVendor.mockRejectedValue("Service method error");

    // Verify that the use case throws an error when service method fails
    await expect(listShopsByVendor.execute(vendorId)).rejects.toThrow('Service method error');
  });
});
