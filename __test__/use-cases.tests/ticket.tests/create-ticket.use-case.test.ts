...
/**
 * Test suite for the CreateTicket use case.
 * This test covers the initialization and behavior of the CreateTicket class.
...
describe('CreateTicket', () => {
  ...
  beforeEach(async () => {
    ...
    createTicket = module.get<CreateTicket>(CreateTicket);
  ...
  it('should create and return an address DTO', async () => {
    ...
    mockTicketService.createTicket.mockResolvedValue(mockTicketDTO);
    (toTicketDTO as jest.Mock).mockReturnValue(mockTicketDTO);

    // Execute the use case with provided parameters
    const result = await createTicket.execute(ticketDTO);

    // Verify that the service was called with the expected arguments
    expect(mockTicketService.createTicket).toHaveBeenCalledWith(ticketDTO);
