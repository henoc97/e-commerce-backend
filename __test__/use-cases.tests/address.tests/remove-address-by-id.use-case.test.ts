...
describe('RemoveAddressById', () => {
  ...
  it('should create and return an address DTO', async () => {
    ...
    mockAddressService.removeAddressById.mockResolvedValue(mockAddressDTO);
    (toAddressDTO as jest.Mock).mockReturnValue(mockAddressDTO);

    // Execute the use case with provided parameters
    const result = await removeAddressById.execute(id);

    // Verify that the service was called with the expected arguments
    expect(mockAddressService.removeAddressById).toHaveBeenCalledWith(id);
   ...
   */
  it('should throw an error when removeAddressById execute method fails', async () => {
    // Simulate a failure when calling the service method
    mockAddressService.removeAddressById.mockRejectedValue("Service method error");

    // Verify that the use case throws an error when service method fails
    await expect(removeAddressById.execute(id)).rejects.toThrow('Service method error');
  });
});
