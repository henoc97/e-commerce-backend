...
/**
 * Test suite for the ListAddressesByState use case.
 * This test covers the initialization and behavior of the ListAddressesByState class.
 * It mocks the AddressService service and verifies that the use case handles the business logic as expected.
 */
describe('ListAddressesByState', () => {
  ...
  it('should create and return an address DTO', async () => {
    ...
    mockAddressService.listAddressesByState.mockResolvedValue(mockAddressDTO);
    (toAddressDTO as jest.Mock).mockReturnValue(mockAddressDTO);

    // Execute the use case with provided parameters
    const result = await listAddressesByState.execute(state);

    // Verify that the service was called with the expected arguments
    expect(mockAddressService.listAddressesByState).toHaveBeenCalledWith(state);
