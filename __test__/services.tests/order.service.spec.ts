...
describe('OrderService', () => {
  ...
  it('should create order', async () => {
    const orderDTO: OrderDTO = {
      ...
      totalAmount: 100.0,
      items: [] // Ajoutez une liste vide ou des éléments fictifs ici
    };

    const returnOject: Order = {
      id: 1,
      userId: 1,
      shopId: 1,
      status: OrderStatus.PENDING,
  ...
  it('should throw an error when get order by id method fails', async () => {
    ...
    // Simulate a failure when calling the repository
    mockOrderRepository.getById.mockRejectedValue(
  ...
  it('should throw an error when update order method fails', async () => {
    ...
    await expect(service.updateOrder(id, updates)).rejects.toThrow(
      'Repository error',
    );
  ...
  it('should throw an error when get orders by user id method fails', async () => {
    ...
    // Simulate a failure when calling the repository
    mockOrderRepository.getByUserId.mockRejectedValue(
      new Error('Repository error'),
    );

    await expect(service.getOrdersByUserId(userId)).rejects.toThrow(
      'Repository error',
    );
  ...
  it('should throw an error when get orders by shop id method fails', async () => {
    ...
    // Simulate a failure when calling the repository
    mockOrderRepository.getByShopId.mockRejectedValue(
      new Error('Repository error'),
    );

    await expect(service.getOrdersByShopId(shopId)).rejects.toThrow(
      'Repository error',
    );
  ...
  it('should throw an error when get orders by status method fails', async () => {
    ...
    // Simulate a failure when calling the repository
    mockOrderRepository.getByStatus.mockRejectedValue(
      new Error('Repository error'),
    );

    await expect(service.getOrdersByStatus(status)).rejects.toThrow(
      'Repository error',
    );
  ...
  it('should throw an error when update order status method fails', async () => {
    ...
    await expect(service.updateOrderStatus(id, status)).rejects.toThrow(
      'Repository error',
    );
  ...
  it('should add payment to order', async () => {
    ...
    expect(result).toEqual(returnOject);
    expect(mockOrderRepository.addPayment).toHaveBeenCalledWith(
      orderId,
      paymentId,
    );
  });
  ...
  it('should throw an error when add payment to order method fails', async () => {
    ...
    // Simulate a failure when calling the repository
    mockOrderRepository.addPayment.mockRejectedValue(
      new Error('Repository error'),
    );

    await expect(service.addPaymentToOrder(orderId, paymentId)).rejects.toThrow(
      'Repository error',
    );
  ...
  it('should add refund to order', async () => {
    ...
    expect(result).toEqual(returnOject);
    expect(mockOrderRepository.addRefund).toHaveBeenCalledWith(
      orderId,
      refundId,
    );
  });
  ...
  it('should throw an error when add refund to order method fails', async () => {
    ...
    // Simulate a failure when calling the repository
    mockOrderRepository.addRefund.mockRejectedValue(
      new Error('Repository error'),
    );

    await expect(service.addRefundToOrder(orderId, refundId)).rejects.toThrow(
      'Repository error',
    );
  ...
  it('should get order by tracking number', async () => {
    ...
    expect(result).toEqual(returnOject);
    expect(mockOrderRepository.getByTrackingNumber).toHaveBeenCalledWith(
      trackingNumber,
    );
  });
  ...
  it('should throw an error when get order by tracking number method fails', async () => {
    ...
    // Simulate a failure when calling the repository
    mockOrderRepository.getByTrackingNumber.mockRejectedValue(
      new Error('Repository error'),
    );

    await expect(
      service.getOrderByTrackingNumber(trackingNumber),
  ...
  it('should get orders by date range', async () => {
    ...
    expect(result).toEqual(returnOject);
    expect(mockOrderRepository.getByDateRange).toHaveBeenCalledWith(
      startDate,
      endDate,
    );
  });
  ...
  it('should throw an error when get orders by date range method fails', async () => {
    ...
    // Simulate a failure when calling the repository
    mockOrderRepository.getByDateRange.mockRejectedValue(
      new Error('Repository error'),
    );
  ...
  it('should throw an error when get recent orders by shop method fails', async () => {
    ...
    // Simulate a failure when calling the repository
    mockOrderRepository.getRecentOrdersByShop.mockRejectedValue(
      new Error('Repository error'),
    );

    await expect(service.getRecentOrdersByShop(shopId, limit)).rejects.toThrow(
      'Repository error',
    );
  ...
  it('should throw an error when get top orders by amount method fails', async () => {
    ...
    // Simulate a failure when calling the repository
    mockOrderRepository.getTopOrdersByAmount.mockRejectedValue(
      new Error('Repository error'),
    );

    await expect(service.getTopOrdersByAmount(topN)).rejects.toThrow(
      'Repository error',
    );
