...
it('should throw an error when create product method fails', async () => {
    ...
    await expect(service.createProduct(product)).rejects.toThrow('Repository error');
...
it('should throw an error when get product by id method fails', async () => {
    ...
    // Simulate a failure when calling the repository 
    mockProductRepository.getById.mockRejectedValue(new Error('Repository error'));

    await expect(service.getProductById(id)).rejects.toThrow('Repository error');
...
it('should update product', async () => {
     ...
     */
     ...
     const id: number = 1;
     const updates: Partial<ProductDTO> = { /* data */ };
...
it('should throw an error when update product method fails', async () => {
    ...
    // Simulate a failure when calling the repository 
    mockProductRepository.update.mockRejectedValue(new Error('Repository error'));

    await expect(service.updateProduct(id,
...
it('should throw an error when delete product method fails', async () => {
    ...
    // Simulate a failure when calling the repository 
    mockProductRepository.delete.mockRejectedValue(new Error('Repository error'));
...
it('should throw an error when find products by name method fails', async () => {
    ...
    // Simulate a failure when calling the repository 
    mockProductRepository.findByName.mockRejectedValue(new Error('Repository error'));

    await expect(service.findProductsByName(name)).rejects.toThrow('Repository error');
...
it('should throw an error when find products by category method fails', async () => {
    ...
    // Simulate a failure when calling the repository 
    mockProductRepository.findByCategory.mockRejectedValue(new Error('Repository error'));

    await expect(service.findProductsByCategory(categoryId)).rejects.toThrow('Repository error');
...
it('should add promotion to product', async () => {
     ...
     */
     ...
     const promotion: PromotionDTO = { /* data */ };

    const returnOject: Product = { id: 1, /* others data */ };
    
    mockProductRepository.addPromotion.mockResolvedValue(returnOject);

    const result = await service.addPromotionToProduct(productId,
    promotion,);
    expect(result).toEqual(returnOject);
    expect(mockProductRepository.addPromotion).toHaveBeenCalledWith(productId,
    promotion,);
});
...
it('should throw an error when add promotion to product method fails', async () => {
    ...
    // Simulate a failure when calling the repository 
    mockProductRepository.addPromotion.mockRejectedValue(new Error('Repository error'));

    await expect(service.addPromotionToProduct(productId,
    promotion,)).rejects.toThrow('Repository error');
...
it('should remove promotion from product', async () => {
    ...
    const result = await service.removePromotionFromProduct(productId,
    promotionId,);
    expect(result).toEqual(returnOject);
    expect(mockProductRepository.removePromotion).toHaveBeenCalledWith(productId,
    promotionId,);
});
...
it('should throw an error when remove promotion from product method fails', async () => {
    ...
    // Simulate a failure when calling the repository 
    mockProductRepository.removePromotion.mockRejectedValue(new Error('Repository error'));

    await expect(service.removePromotionFromProduct(productId,
...
it('should add image to product', async () => {
     ...
     */
     ...
     const productId: number = 1;
     const image: ProductImageDTO = { /* data */ };

    const returnOject: Product = { id: 1, /* others data */ };
    
    mockProductRepository.addImage.mockResolvedValue(returnOject);

    const result = await service.addImageToProduct(productId,
    image,);
    expect(result).toEqual(returnOject);
    expect(mockProductRepository.addImage).toHaveBeenCalledWith(productId,
    image,);
});
...
it('should throw an error when add image to product method fails', async () => {
    ...
    // Simulate a failure when calling the repository 
    mockProductRepository.addImage.mockRejectedValue(new Error('Repository error'));

    await expect(service.addImageToProduct(productId,
    image,)).rejects.toThrow('Repository error');
...
it('should remove image from product', async () => {
    ...
    const result = await service.removeImageFromProduct(productId,
    imageId,);
    expect(result).toEqual(returnOject);
    expect(mockProductRepository.removeImage).toHaveBeenCalledWith(productId,
    imageId,);
});
...
it('should throw an error when remove image from product method fails', async () => {
    ...
    // Simulate a failure when calling the repository 
    mockProductRepository.removeImage.mockRejectedValue(new Error('Repository error'));

    await expect(service.removeImageFromProduct(productId,
    imageId,)).rejects.toThrow('Repository error');
...
it('should add variant to product', async () => {
     ...
     */
     ...
     const productId: number = 1;
     const variant: ProductVariantDTO = { /* data */ };

    const returnOject: Product = { id: 1, /* others data */ };
    
    mockProductRepository.addVariant.mockResolvedValue(returnOject);

    const result = await service.addVariantToProduct(productId,
    variant,);
    expect(result).toEqual(returnOject);
    expect(mockProductRepository.addVariant).toHaveBeenCalledWith(productId,
    variant,);
});
...
it('should throw an error when add variant to product method fails', async () => {
    ...
    // Simulate a failure when calling the repository 
    mockProductRepository.addVariant.mockRejectedValue(new Error('Repository error'));

    await expect(service.addVariantToProduct(productId,
    variant,)).rejects.toThrow('Repository error');
...
it('should remove variant from product', async () => {
    ...
    const result = await service.removeVariantFromProduct(productId,
    variantId,);
    expect(result).toEqual(returnOject);
    expect(mockProductRepository.removeVariant).toHaveBeenCalledWith(productId,
    variantId,);
});
...
it('should throw an error when remove variant from product method fails', async () => {
    ...
    // Simulate a failure when calling the repository 
    mockProductRepository.removeVariant.mockRejectedValue(new Error('Repository error'));

    await expect(service.removeVariantFromProduct(productId,
    variantId,)).rejects.toThrow('Repository error');
...
it('should update product stock', async () => {
    ...
    const result = await service.updateProductStock(productId,
    quantity,);
    expect(result).toEqual(returnOject);
    expect(mockProductRepository.updateStock).toHaveBeenCalledWith(productId,
    quantity,);
});
...
it('should throw an error when update product stock method fails', async () => {
    ...
    // Simulate a failure when calling the repository 
    mockProductRepository.updateStock.mockRejectedValue(new Error('Repository error'));

    await expect(service.updateProductStock(productId,
    quantity,)).rejects.toThrow('Repository error');
...
it('should add review to product', async () => {
    ...
    const result = await service.addReviewToProduct(productId,
    review,);
    expect(result).toEqual(returnOject);
    expect(mockProductRepository.addReview).toHaveBeenCalledWith(productId,
    review,);
});
...
it('should throw an error when add review to product method fails', async () => {
    ...
    // Simulate a failure when calling the repository 
    mockProductRepository.addReview.mockRejectedValue(new Error('Repository error'));

    await expect(service.addReviewToProduct(productId,
    review,)).rejects.toThrow('Repository error');
...
it('should throw an error when get product reviews method fails', async () => {
    ...
    // Simulate a failure when calling the repository 
    mockProductRepository.getReviews.mockRejectedValue(new Error('Repository error'));

    await expect(service.getProductReviews(productId)).rejects.toThrow('Repository error');
...
it('should add cart item to product', async () => {
     ...
     */
     ...
     const productId: number = 1;
     const cartItem: CartItemDTO = { /* data */ };

    const returnOject: Product = { id: 1, /* others data */ };
    
    mockProductRepository.addCartItem.mockResolvedValue(returnOject);

    const result = await service.addCartItemToProduct(productId,
    cartItem,);
    expect(result).toEqual(returnOject);
    expect(mockProductRepository.addCartItem).toHaveBeenCalledWith(productId,
    cartItem,);
});
...
it('should throw an error when add cart item to product method fails', async () => {
    ...
    // Simulate a failure when calling the repository 
    mockProductRepository.addCartItem.mockRejectedValue(new Error('Repository error'));

    await expect(service.addCartItemToProduct(productId,
    cartItem,)).rejects.toThrow('Repository error');
...
it('should find products by price range', async () => {
    ...
    const result = await service.findProductsByPriceRange(minPrice,
    maxPrice,);
    expect(result).toEqual(returnOject);
    expect(mockProductRepository.findByPriceRange).toHaveBeenCalledWith(minPrice,
    maxPrice,);
});
...
it('should throw an error when find products by price range method fails', async () => {
    ...
    // Simulate a failure when calling the repository 
    mockProductRepository.findByPriceRange.mockRejectedValue(new Error('Repository error'));

    await expect(service.findProductsByPriceRange(minPrice,
...
it('should throw an error when get featured products method fails', async () => {
    ...
    // Simulate a failure when calling the repository 
    mockProductRepository.getFeaturedProducts.mockRejectedValue(new Error('Repository error'));

    await expect(service.getFeaturedProducts()).rejects.toThrow('Repository error');
