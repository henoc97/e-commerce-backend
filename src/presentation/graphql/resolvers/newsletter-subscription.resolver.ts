...
export class NewsletterSubscriptionResolver {
  ...
  @Query(() => Boolean)
  async checkEmailSubscribed(@Args('email') email: string, @Args('shopId') shopId: number): Promise<boolean> {
    return this.checkEmailSubscribedUseCase.execute(email, shopId);
  }

  @Query(() => Number)
  async countTotalNewsletterSubscriptions(@Args('shopId') shopId: number): Promise<number> {
    return this.countTotalNewsletterSubscriptionsUseCase.execute(shopId);
  }

  @Query(() => NewsletterSubscriptionDTO, { nullable: true })
  async fetchNewsletterSubscriptionByEmail(@Args('email') email: string, @Args('shopId') shopId: number): Promise<NewsletterSubscriptionDTO | null> {
    return this.fetchNewsletterSubscriptionByEmailUseCase.execute(email, shopId);
  }

  @Query(() => NewsletterSubscriptionDTO, { nullable: true })
  async fetchNewsletterSubscriptionById(@Args('id') id: number): Promise<NewsletterSubscriptionDTO | null> {
  ...
  async fetchNewsletterSubscriptionsByDateRange(
    ...
    return this.fetchNewsletterSubscriptionsByDateRangeUseCase.execute(startDate, endDate, shopId);
  }

  @Query(() => [NewsletterSubscriptionDTO])
  async listNewsletterSubscriptions(@Args('shopId') shopId: number): Promise<NewsletterSubscriptionDTO[]> {
    return this.listNewsletterSubscriptionsUseCase.execute(shopId);
  }

  @Mutation(() => NewsletterSubscriptionDTO, { nullable: true })
  async createNewsletterSubscription(@Args('dto') dto: NewsletterSubscriptionDTO): Promise<NewsletterSubscriptionDTO | null> {
  ...
  @Mutation(() => NewsletterSubscriptionDTO, { nullable: true })
  async updateNewsletterSubscription(@Args('id') id: number, @Args('dto') dto: Partial<NewsletterSubscriptionDTO>): Promise<NewsletterSubscriptionDTO | null> {
